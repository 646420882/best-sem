        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.length == 0) {
                    ...
                    $("#showA").empty();
                    var url = "/assistantKeyword/getSearchWordReport?levelOfDetails=" + levelOfDetails + "&startDate=" + startDate +
                            "&endDate=" + endDate + "&device=" + device + "&attributes=" + regions + "&downStatus=down"
                    $("#showA").append("<a style='margin-left: 25%;' href='" + url + "'  class='become fl'>下载报告</a>")
                }
            }
        });
    });
