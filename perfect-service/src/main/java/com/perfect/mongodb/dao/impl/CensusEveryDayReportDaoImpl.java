...
public class CensusEveryDayReportDaoImpl extends AbstractSysBaseDAOImpl implements CensusEveryDayReportDao {
    ...
    public List<ViewsDTO> getGroupLastPageByDate(Date date){
        ...
        AggregationResults<ViewsDTO> results = mongoTemplate.aggregate(agg, EntityConstants.SYS_CENSUS, ViewsDTO.class);
    ...
    public long getCensusCount(Date date, String lastPage,String database_Field) {
        ...
        long count;
        if(aggre!=null){
            AggregationResults<ViewsDTO> results = mongoTemplate.aggregate(aggre, EntityConstants.SYS_CENSUS, ViewsDTO.class);
            count = results.getMappedResults().size();
        }else{
            count = mongoTemplate.count(q,EntityConstants.SYS_CENSUS);
        }
