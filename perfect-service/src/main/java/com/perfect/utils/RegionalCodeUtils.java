...
import com.perfect.dto.RedisRegional;
...
public class RegionalCodeUtils {
     ...
     * @return
     */
    public static Map<Integer, List<RedisRegional>> provinceCode(List<Integer> id) {
        Jedis jc = JRedisUtils.get();
        String data = jc.get("RegionalCodeRedis");
        Gson gson = new Gson();
        ...
        Map<Integer, List<RedisRegional>> map = new HashMap<>();

        for (Integer integer : id) {

            List<RedisRegional> redisRegionalList = new ArrayList<>();

            for (Map.Entry<Integer, String> voEntity : mapKey.entrySet()) {

                RedisRegional redisRegional = new RedisRegional();
