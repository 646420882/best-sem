$.fn.extend({
	grid:function(cfg){
		...
		function gridView(_this,config){
			_this.addClass("grid-table").css("width","100%");
			_this.find("th:eq(0)").css("borderLeft","0px");
			_this.css("height",config.height);
			...
			if(_this.find(".grid-view-td").size()==0){
				...
				appendTo(_this).height(config.height-ths.height()-30);
				...
				_this.hover(function(){
					...
					scrollOffset = isNaN(parseInt(scrollOffset))?0:Math.abs(parseInt(scrollOffset));
					leftBtn.css({
						left:_this.offset().left-15,
						top:_this.offset().top+scrollOffset+_this.height()/2-29
					});
					rightBtn.css({
						left:_this.offset().left+(_this.width()-15),
						top:_this.offset().top+scrollOffset+_this.height()/2-29
					});
					rightBtn.show();
					leftBtn.show();
			...
			function gridClass(){
				this.load = function (start){
					...
					if(maxPage==0||(start!=currentPage&&start>=1&&start<=maxPage)){
						param.start = start;
						$.post(config.url,param,function(json){
							var pager = eval("("+json+")");
